...
class ShopPage:
    ...
    def authorization(self):
        with allure.step("Находим поле предназначенное для логина, проверяем что оно кликабельно"):
        ...
        with allure.step("Находим поле предназначенное для Пароля, проверяем что оно кликабельно"):
        ...
        with allure.step("проверяем что кнопка Залогинится активна, прокликиваем её"):
    ...
    def shop_tri(self):
        ...
        for product_name in products_to_add:
            with allure.step("Поиск товаров по названию из списка {products_to_add}"):
                product_card = self.wait.until(
                    EC.visibility_of_element_located(
                        (By.XPATH,
                            f"//div[@class='inventory_item']//div[text()='{product_name}']"
                            f"/ancestor::div[@class='inventory_item']")
                )
            )
            with allure.step("Прокликиваем мышью по товару добавляя его в корзину"):
                add_btn = product_card.find_element(By.CSS_SELECTOR, "button.btn_primary")
                add_btn.click()
        with allure.step("Находим элемент корзина, проверяем что он виден"):
            cart_badge = self.wait.until(EC.visibility_of_element_located(self.CART_BADGE))
        with allure.step("Кликаем на корзину"):
            cart_badge.click()
        with allure.step("Проверяем что кнопка продолжить кликабельна"):
            checkout_link = self.wait.until(EC.element_to_be_clickable(self.CHECKOUT_LINK))
        with allure.step("Прокликиваем мышью по кнопке продолжить"):
            checkout_link.click()
